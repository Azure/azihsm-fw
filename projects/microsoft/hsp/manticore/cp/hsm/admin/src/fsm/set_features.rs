...
pub(crate) struct AdminSetFeaturesCmd<E: AdminEnvTrait + 'static> {
    /// Pcie function
    pfn: PcieFunction,
...
impl<E: AdminEnvTrait> AdminCmdTrait<E> for AdminSetFeaturesCmd<E> {
...
impl<E: AdminEnvTrait> AdminSetFeaturesCmd<E> {
    /// Create a new Admin Set Features command FSM
    pub fn new(
        ctx: AdminFsmContext<E>,
        pfn: PcieFunction,
        sqe: AdminSqe,
    ...
    fn on_start(&mut self, tag: TagId) -> Result<(), AdminErr> {
        ...
        // Set command specific parameters for the given feature based on the number of
        // resources
        let set_features_sqe: SetFeaturesSqe = self.sqe.into();
        let cmd_specific_data = self
            .set_cmd_specific_data(set_features_sqe.id, num_res)
            .or_else(|err| {
                error!("[set_features] Invalid SQE: {:?}", err as u32);
    ...
    /// Set the command specific parameters for the given feature
    fn set_cmd_specific_data(
        &mut self,
        ...
        // AdminFeatureId::RtFwCapabilities is not allowed to be set
        match feature_id {
