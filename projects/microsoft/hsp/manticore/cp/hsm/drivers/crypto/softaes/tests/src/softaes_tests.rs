...
fn test_aes(op: AesOp, message: &[u8], key: &[u8], expected: &[u8]) {
    let result_buf = create_result_buff(expected.len());
    result_buf[0..message.len()].copy_from_slice(message);
    let aes = SoftAes::new();
    let status = match op {
        AesOp::Encrypt => aes.encrypt(key, result_buf),
        AesOp::Decrypt => aes.decrypt(key, result_buf),
    };
    assert_eq!(status, Ok(()));
...
fn test_aes_ecb_128_encrypt() {
    ...
    test_aes(AesOp::Encrypt, &message, &key, &expected);
}
...
fn test_aes_ecb_128_decrypt() {
    ...
    test_aes(AesOp::Decrypt, &message, &key, &expected);
}
...
fn test_invalid_message_len() {
    let aes = SoftAes::new();
    let status = aes.encrypt(&[], &mut [0u8; 5]);
    assert_eq!(status, Err(0x000C0005));
    let status = aes.decrypt(&[], &mut [0u8; 5]);
    assert_eq!(status, Err(0x000C0005));
}

fn test_invalid_key_len() {
    let aes = SoftAes::new();
    let status = aes.encrypt(&[], &mut [0u8; 16]);
    assert_eq!(status, Err(0x000C0002));
    let status = aes.decrypt(&[], &mut [0u8; 16]);
    assert_eq!(status, Err(0x000C0002));
}
