...
enum State {
    ...
    WaitForEngineToBeginEstablishCredEncKey,

    /// Wait for completion of PKA operation for getting
    WaitForEndGetEstablishCredEncKey,
...
/// Get establish credential encryption key id command
pub(crate) struct GetEstablishCredEncryptionKeyCmd<E: HsmEnvTrait + 'static> {
    /// Current state
    state: State,
    ...
    /// Command response
    cmd_resp: Option<DdiGetEstablishCredEncryptionKeyCmdResp>,

    /// Get Establish Cred Encryption Key command context
    cmd_ctx: Option<
        GetEstablishCredEncryptionKeyCtx<
...
impl<E: HsmEnvTrait> HsmCmdTrait<E> for GetEstablishCredEncryptionKeyCmd<E> {
    ...
    fn on_event(&mut self, event: HsmFsmEvent, tag: TagId) -> Result<(), HsmErr> {
        match (self.state, event) {
            ...
            (State::WaitForEngineToBeginEstablishCredEncKey, HsmFsmEvent::ResourceReady(_)) => {
                self.begin_get_establish_cred_encryption_key(tag)
            }
            ...
            (State::WaitForEndGetEstablishCredEncKey, HsmFsmEvent::PkaDone(_))
            | (State::WaitForEndGetEstablishCredEncKey, HsmFsmEvent::PkaError(_)) => {
                self.end_get_establish_cred_encryption_key(tag)
            }
    ...
    fn rollback(&mut self, _tag: TagId) -> HsmResult<()> {
        if let Some(key_id) = self.key_id {
            ...
            self.part.unset_establish_cred_encryption_key_id();
        }
...
impl<E: HsmEnvTrait> GetEstablishCredEncryptionKeyCmd<E> {
    ...
    fn on_start(&mut self, tag: TagId) -> Result<(), HsmErr> {
        ...
        let req = decode_buf::<DdiGetEstablishCredEncryptionKeyCmdReq, E>(&self.req)?;
    ...
    /// Helper to begin the process of get establish cred encryption key
    fn begin_get_establish_cred_encryption_key(&mut self, tag: TagId) -> Result<(), HsmErr> {
        match self.part.begin_get_establish_cred_encryption_key(tag) {
            Ok(ctx) => {
                if let Some(key_data) = ctx.key_data {
                    ...
                    self.state = State::WaitForEndGetEstablishCredEncKey;
            ...
            Err(mut err) => {
                if err.pending() && self.state == State::Init {
                    self.state = State::WaitForEngineToBeginEstablishCredEncKey
                } else if err.pending()
                    && self.state == State::WaitForEngineToBeginEstablishCredEncKey
                {
                    self.state = State::Final;
    ...
    /// On PKA cmd complete
    fn end_get_establish_cred_encryption_key(&mut self, tag: TagId) -> Result<(), HsmErr> {
        let ctx = self.cmd_ctx.take().ok_or(HsmErr::InvalidState)?;
        let out = self
            .part
            .end_get_establish_cred_encryption_key(ctx.tag, ctx)?;
                ...
                Ok(success) => {
                    if !success {
                        self.part.notify_pct_validation_failure(
                            HsmErr::PctValidationEstablishCredEncKeyFailed as u32,
                        );
                    }
                }
    ...
    fn check_alive(&mut self) -> Result<(), HsmErr> {
        if (self.state == State::WaitForEngineToBeginEstablishCredEncKey
            || self.state == State::WaitForEndGetEstablishCredEncKey
            || self.state == State::WaitForEngineToBeginPct
            || self.state == State::WaitForPctValidaiton
            || self.state == State::WaitForEngineToBeginEccSign
    ...
    fn cmd_resp(
        ...
        DdiGetEstablishCredEncryptionKeyCmdResp {
            ...
            data: DdiGetEstablishCredEncryptionKeyResp {
