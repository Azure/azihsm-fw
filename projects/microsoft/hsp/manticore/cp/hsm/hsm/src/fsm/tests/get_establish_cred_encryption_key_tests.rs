...
use super::*;
use crate::fsm::get_establish_cred_encryption_key::GetEstablishCredEncryptionKeyCmd;
use crate::fsm::CredentialEncryptionKeyData;
use crate::fsm::GetEstablishCredEncryptionKeyCtx;
...
fn test_invalid_event() {
    ...
    let req = encode_buf::<DdiGetEstablishCredEncryptionKeyCmdReq, _>(&cmd(), &heap).unwrap();

    let mut cmd = GetEstablishCredEncryptionKeyCmd::<MockEnv>::new(req, heap, part);
    assert_eq!(
...
fn test_encode_buf_err() {
    ...
    part.expect_begin_get_establish_cred_encryption_key()
        .once()
        .returning(|_| Ok(pending_ctx()));
    part.expect_end_get_establish_cred_encryption_key()
        .once()
        .returning(|_, _| {
            Ok(GetEncryptionKeyOut {
    ...
    part.expect_delete_internal_key()
        .once()
        .returning(|_| Ok(()));
    part.expect_unset_establish_cred_encryption_key_id()
        .once()
        .returning(|| ());
...
#[test]
fn test_get_establish_cred_encryption_key_new_key() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
    ...
    let resp =
        decode_buf::<DdiGetEstablishCredEncryptionKeyCmdResp, MockEnv>(&resp.unwrap()).unwrap();
...
#[test]
fn test_get_establish_cred_encryption_key_on_resource_ready() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
#[test]
fn test_get_establish_cred_encryption_key_on_resource_ready_error() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(1)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
#[test]
fn test_get_establish_cred_encryption_key_existing_key() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
fn test_requires_resource() {
    ...
    let req = encode_buf::<DdiGetEstablishCredEncryptionKeyCmdReq, _>(&cmd(), &heap).unwrap();
    let cmd = GetEstablishCredEncryptionKeyCmd::<MockEnv>::new(req, heap, part);
...
#[test]
fn test_get_establish_cred_encryption_key_new_key_bad_event() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
#[test]
fn test_get_establish_cred_encryption_begin_error() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(1)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
fn pending_ctx() -> GetEstablishCredEncryptionKeyCtx<MockEnv> {
    let scheduler = CmdScheduler::new(128, 1, HsmFsmEventRecorder::default());
    let resource = CmdResource::new(PkaResource::new(vec![MockPka::new()]), scheduler, 1);
...
fn final_ctx() -> GetEstablishCredEncryptionKeyCtx<MockEnv> {
    GetEstablishCredEncryptionKeyCtx {
        tag: TagId::default(),
        engine_ref: None,
        cmd_info: None,
...
fn cmd() -> DdiGetEstablishCredEncryptionKeyCmdReq {
    DdiGetEstablishCredEncryptionKeyCmdReq {
        hdr: DdiReqHdr {
