...
fn test_invalid_event() {
    ...
    let req = encode_buf::<DdiKbkdfCounterHmacDeriveCmdReq, _>(&cmd_req(), &heap).unwrap();

    let mut cmd = KbkdfDeriveCmd::<MockEnv>::new(req, heap, app_session, part, PcieFunction::Vf0);
    assert_eq!(
...
#[test]
fn test_kbkdf_derive_aesbulk256_key() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
    ...
    app_session
        .expect_begin_kbkdf_aesbulk256_derive()
        .once()
    ...
    let req =
        encode_buf::<DdiKbkdfCounterHmacDeriveCmdReq, _>(&aesbulk256_cmd_req(), &heap).unwrap();
    let mut cmd = KbkdfDeriveCmd::<MockEnv>::new(req, heap, app_session, part, PcieFunction::Vf0);
    assert_eq!(
        cmd.on_event(HsmFsmEvent::StartCmd, TagId::default()),
        Err(HsmErr::Pending)
...
#[test]
fn test_kbkdf_derive_aesbulk256_key_on_engine_ready() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
#[test]
fn test_kbkdf_derive_aesbulk256_key_on_engine_ready_err_pending() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(1)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
#[test]
fn test_kbkdf_derive_aesbulk256_key_on_engine_ready_err() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(1)
        .returning(|s| Some(MockDmaAlloc::new(s)));
...
#[test]
fn test_kbkdf_derive_some_label_context() {
    let mut heap = MockDmaHeap::new();
    heap.expect_allocate_from_pool()
        .times(2)
        .returning(|s| Some(MockDmaAlloc::new(s)));
    ...
    let label = MborByteArray::new_with_len([0u8; 256].as_ptr(), 256);
    let context = MborByteArray::new_with_len([0u8; 256].as_ptr(), 256);
    cmd_req.data.label = Some(label);
    cmd_req.data.context = Some(context);
...
fn test_requires_resource() {
    ...
    let req = encode_buf::<DdiKbkdfCounterHmacDeriveCmdReq, _>(&cmd_req(), &heap).unwrap();
    let app_session = MockUserSession::new();
    let cmd = KbkdfDeriveCmd::<MockEnv>::new(req, heap, app_session, part, PcieFunction::Vf0);

    assert!(!cmd.requires_resource(TagId::default(), HsmFsmResourceId::FpIpcChannel));
}

fn cmd_req() -> DdiKbkdfCounterHmacDeriveCmdReq {
    DdiKbkdfCounterHmacDeriveCmdReq {
        hdr: DdiReqHdr {
...
fn aesbulk256_cmd_req() -> DdiKbkdfCounterHmacDeriveCmdReq {
    DdiKbkdfCounterHmacDeriveCmdReq {
        hdr: DdiReqHdr {
            rev: Some(DdiApiRev { major: 1, minor: 0 }),
            op: DdiOp::KbkdfCounterHmacDerive,
...
fn validate_response(resp: Option<MockDmaAlloc>, aes_bulk256: bool) {
    let resp = decode_buf::<DdiKbkdfCounterHmacDeriveCmdResp, MockEnv>(&resp.unwrap()).unwrap();
