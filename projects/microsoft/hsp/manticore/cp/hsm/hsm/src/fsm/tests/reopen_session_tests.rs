...
fn test_invalid_event() {
    ...
    let req = encode_buf::<DdiReopenSessionCmdReq, _>(&cmd_req(), &heap).unwrap();

    let mut cmd = OpenSessionCmd::<MockEnv>::new(req, heap, part, true);
    assert_eq!(
...
fn test_encode_buf_err() {
    ...
    part.expect_cmd_fsm_test_action()
        .times(1)
        .returning(|_| None);
...
fn test_open_app_session() {
    ...
    let resp = decode_buf::<DdiReopenSessionCmdResp, MockEnv>(&resp.unwrap()).unwrap();
...
fn cmd_req() -> DdiReopenSessionCmdReq {
    DdiReopenSessionCmdReq {
        hdr: DdiReqHdr {
        ...
        data: DdiReopenSessionReq {
            ...
            bmk_session: MborByteArray::new_with_len(core::ptr::null(), 1024),
        },
    }
}
