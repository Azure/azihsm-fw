// Copyright (c) Microsoft Corporation. All rights reserved.

use mcr_ddi_types::DdiApiRev;

/// Minimum API revision for test.
pub(crate) const MIN_API_REV_TEST: DdiApiRev = DdiApiRev { major: 1, minor: 0 };

/// Maximum API revision for test.
pub(crate) const MAX_API_REV_TEST: DdiApiRev = DdiApiRev { major: 1, minor: 0 };

/// RSA 2K public DER key in to be used for test.
#[allow(unused)]
pub(crate) const TEST_RSA_2K_PUBLIC_KEY_DER: [u8; 294] = [
    ,
];

/// RSA 2K private key to be used for test.
pub(crate) const TEST_RSA_2K_PRIVATE_KEY: [u8; 1214] = [
    ,
];

/// RSA 2K public key to be used for test.
pub(crate) const TEST_RSA_2K_PUBLIC_KEY: [u8; 294] = [
    ,
];

/// ECC 256 private key to be used for test.
pub(crate) const TEST_ECC_256_PRIVATE_KEY: [u8; 138] = [
    ,
];

/// RAW ECC 256 public key to be used for test.
pub(crate) const TEST_RAW_ECC_256_PUBLIC_KEY: [u8; 64] = [
    0xb0, 0x51, 0x4f, 0x91, 0x20, 0x67, 0x1e, 0xd0, 0xfd, 0xab, 0x69, 0x81, 0x8f, 0xd3, 0x67, 0xca,
    0xc1, 0x8b, 0x4e, 0x8b, 0x20, 0xf2, 0xa4, 0x3b, 0xb8, 0x08, 0xea, 0xa1, 0xcd, 0xc0, 0x1a, 0xa8,
    0x22, 0x89, 0x40, 0x9a, 0xab, 0xc6, 0x30, 0xe8, 0x0e, 0x4a, 0x16, 0x47, 0xbf, 0x94, 0x43, 0x2e,
    0xd5, 0xa3, 0x41, 0x3a, 0xd4, 0x3e, 0x79, 0x41, 0xc1, 0x20, 0xf1, 0x56, 0x23, 0x0f, 0x51, 0x0b,
];

/// RSA 3K private key to be used for test.
pub(crate) const TEST_RSA_3K_PRIVATE_KEY: [u8; 1793] = [
    ,
];

/// RSA 3K public key to be used for test.
pub(crate) const TEST_RSA_3K_PUBLIC_KEY: [u8; 422] = [
    ,
];

/// RSA 4K private key to be used for test.
pub(crate) const TEST_RSA_4K_PRIVATE_KEY: [u8; 2375] = [
    ,
];

/// RSA 4k public key to be used for test.
pub(crate) const TEST_RSA_4K_PUBLIC_KEY: [u8; 550] = [
    ,
];

/// ECC 384 private key to be used for test.
#[allow(unused)]
pub(crate) const TEST_ECC_384_PRIVATE_KEY: [u8; 185] = [
    ,
];

/// ECC 384 public key to be used for test.
#[allow(unused)]
pub(crate) const TEST_ECC_384_PUBLIC_KEY: [u8; 120] = [
    0x30, 0x76, 0x30, 0x10, 0x06, 0x07, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x02, 0x01, 0x06, 0x05, 0x2b,
    0x81, 0x04, 0x00, 0x22, 0x03, 0x62, 0x00, 0x04, 0xe4, 0x20, 0x9a, 0xd7, 0x07, 0xa4, 0x88, 0x1a,
    0xff, 0xf0, 0x12, 0x61, 0x92, 0xc7, 0x9d, 0x83, 0x77, 0x49, 0x21, 0xcc, 0x5d, 0xf3, 0xb9, 0x21,
    0xc4, 0x3d, 0xae, 0xaa, 0x58, 0xb8, 0x34, 0x2b, 0x38, 0x3c, 0xda, 0xb2, 0x88, 0xf0, 0xe4, 0xb9,
    0x56, 0x14, 0x11, 0x15, 0x75, 0xba, 0xbb, 0x23, 0x7c, 0x67, 0xf7, 0xd1, 0x97, 0x63, 0xc7, 0xb8,
    0x56, 0xd3, 0x22, 0xb2, 0xba, 0xba, 0x1a, 0xc6, 0xb4, 0xea, 0x0d, 0xad, 0xa2, 0x56, 0x29, 0xd5,
    0xca, 0x0f, 0x4a, 0x4e, 0xee, 0x17, 0xb0, 0xb2, 0xf4, 0xb1, 0x58, 0xba, 0xae, 0xa1, 0x58, 0x9c,
    0x10, 0x07, 0xf7, 0x0e, 0xc7, 0x62, 0x42, 0xe0,
];

/// Raw ECC 384 public key to be used for test.
#[allow(unused)]
pub(crate) const TEST_RAW_ECC_384_PUBLIC_KEY: [u8; 96] = [
    0xe4, 0x20, 0x9a, 0xd7, 0x07, 0xa4, 0x88, 0x1a, 0xff, 0xf0, 0x12, 0x61, 0x92, 0xc7, 0x9d, 0x83,
    0x77, 0x49, 0x21, 0xcc, 0x5d, 0xf3, 0xb9, 0x21, 0xc4, 0x3d, 0xae, 0xaa, 0x58, 0xb8, 0x34, 0x2b,
    0x38, 0x3c, 0xda, 0xb2, 0x88, 0xf0, 0xe4, 0xb9, 0x56, 0x14, 0x11, 0x15, 0x75, 0xba, 0xbb, 0x23,
    0x7c, 0x67, 0xf7, 0xd1, 0x97, 0x63, 0xc7, 0xb8, 0x56, 0xd3, 0x22, 0xb2, 0xba, 0xba, 0x1a, 0xc6,
    0xb4, 0xea, 0x0d, 0xad, 0xa2, 0x56, 0x29, 0xd5, 0xca, 0x0f, 0x4a, 0x4e, 0xee, 0x17, 0xb0, 0xb2,
    0xf4, 0xb1, 0x58, 0xba, 0xae, 0xa1, 0x58, 0x9c, 0x10, 0x07, 0xf7, 0x0e, 0xc7, 0x62, 0x42, 0xe0,
];

#[allow(unused)]
pub(crate) const TEST_RAW_ECC_521_PUBLIC_KEY: [u8; 136] = [
    ,
];

pub(crate) const TEST_CERTIFICATE: [u8; 679] = [
    ,
];

pub(crate) const PUB_KEY_RAW: [u8; 97] = [
    0x04, // Uncompressed point indicator
    // X coordinate (48 bytes)
    0xd9, 0x7a, 0x10, 0x66, 0xf9, 0x05, 0xf1, 0x02, 0x69, 0x52, 0x3c, 0x05, 0x5e, 0x3e, 0x58, 0xbd,
    0x1f, 0x29, 0x27, 0x79, 0x08, 0xb8, 0xde, 0xdd, 0xbc, 0x2f, 0xe2, 0xd1, 0xfc, 0x98, 0x5f, 0x6c,
    0x2b, 0x62, 0xf5, 0xa9, 0x1e, 0x5b, 0x37, 0xc7, 0x16, 0xdf, 0x16, 0x3a, 0xbc, 0x9f, 0x68,
    0x67, // Y coordinate (48 bytes)
    0x6b, 0x2f, 0xd4, 0x3e, 0x29, 0x2b, 0xc9, 0x87, 0xd5, 0xc1, 0x3d, 0x7c, 0x6e, 0x61, 0x47, 0x79,
    0x9f, 0x5f, 0x2c, 0x5f, 0x6a, 0x8b, 0xf6, 0x43, 0xf8, 0xb5, 0x8e, 0xe6, 0x27, 0x99, 0x4d, 0xeb,
    0x04, 0x76, 0x74, 0x5f, 0x23, 0x2b, 0x8a, 0x12, 0xea, 0xf2, 0xf7, 0xc4, 0x4f, 0x5c, 0x1f, 0x6a,
];
