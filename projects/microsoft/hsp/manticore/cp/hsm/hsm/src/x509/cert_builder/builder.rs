...
//! X509 API to construct a Certificate or Certificate Signing Request
//! from a "To Be Signed" blob and an ECDSA-384 Signature.

use crate::{error::HsmErr, partition::EccSignature};

/// Type alias for ECDSA-384 CSR Builder
...
impl TryFrom<EccSignature> for Ecdsa384Signature {
    type Error = HsmErr;

    fn try_from(value: EccSignature) -> Result<Self, Self::Error> {
        if value.r.len() >= Self::ECDSA_COORD_LEN && value.s.len() >= Self::ECDSA_COORD_LEN {
            Ok(Self {
                r: value.r().try_into().map_err(|_| HsmErr::InvalidArgument)?,
                s: value.s().try_into().map_err(|_| HsmErr::InvalidArgument)?,
            })
        } else {
