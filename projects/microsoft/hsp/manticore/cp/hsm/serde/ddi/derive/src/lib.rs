...
fn on_enum(input: &mut syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    let ddi_enum = r#enum::prase_enum(input)?;
    let len = len::enum_len(&ddi_enum)?;
    let encode = encode::enum_encode(&ddi_enum)?;
    let decode = decode::enum_decode(&ddi_enum)?;
