...
pub enum DdiOp {
    ...
    /// Import private key in RAW format for testing
    RawKeyImport = 2008,
...
pub enum DdiStatus {
    ...
    /// Another key with the same name already exists
    ...
    /// Partition Id generation failed
    PartitionIdGenerationFailed = 141557945,

    /// Partition Cert generation failed
    PartitionCertGenerationFailed = 141557946,

    /// Partition certificate creation failed due to invalid type conversion
    PartitionCertInvalidTypeConversion = 141557947,

    /// FSM allocation failed
    FsmAllocationFailed = 141557948,

    /// Partition certificate too large
    PartitionCertTooLarge = 141557949,
...
pub enum DdiKeyClass {
    ...
    /// AES GCM Unapproved Bulk
    AesGcmUnApprovedBulk = 6,
...
impl DdiKeyType {
    pub fn to_public(&self) -> Self {
        match *self {
            DdiKeyType::Rsa2kPrivate | DdiKeyType::Rsa2kPrivateCrt => DdiKeyType::Rsa2kPublic,
            DdiKeyType::Rsa3kPrivate | DdiKeyType::Rsa3kPrivateCrt => DdiKeyType::Rsa3kPublic,
            DdiKeyType::Rsa4kPrivate | DdiKeyType::Rsa4kPrivateCrt => DdiKeyType::Rsa4kPublic,
            DdiKeyType::Ecc256Private => DdiKeyType::Ecc256Public,
            DdiKeyType::Ecc384Private => DdiKeyType::Ecc384Public,
            DdiKeyType::Ecc521Private => DdiKeyType::Ecc521Public,
            other_key_type => other_key_type,
        }
    }
...
impl From<DdiOp> for DdiSessionKind {
    ...
    fn from(value: DdiOp) -> Self {
        match value {
            ...
            | DdiOp::GetCertificate => DdiSessionKind::None,
...
impl DdiDerPublicKey {
    ...
    pub fn pub_key_der_post_decode(
        ...
        let output_vec = match self.key_kind {
            DdiKeyType::Rsa2kPublic | DdiKeyType::Rsa3kPublic | DdiKeyType::Rsa4kPublic => {
                ...
                let key_data = RsaPublicKeyData {
                    e: input_array.data[n_len..n_len + e_len].to_vec(),
                    n: input_array.data[..n_len].to_vec(),
                    little_endian: true,
                };
            ...
            DdiKeyType::Ecc256Public | DdiKeyType::Ecc384Public | DdiKeyType::Ecc521Public => {
                ...
                reverse_copy(&mut x, &input_array.data[..der_curve_len]);

                let mut y = [0u8; MAX_ECC_DER_COMPONENT_SIZE];
                reverse_copy(
                    &mut y,
                    &input_array.data[pka_curve_len..pka_curve_len + der_curve_len],
                );
        ...
        let output = MborByteArray::new(output_array, output_vec.len());
        Ok(output)
    }
    ...
    pub fn pub_key_der_pre_encode(
        ...
        let output = MborByteArray::new(output_array, pka_curve_len * 2);
        Ok(output)
    }
}
...
impl From<DdiOp> for u32 {
    fn from(op: DdiOp) -> Self {
        match op {
            // Invalid operation
            DdiOp::Invalid => 1001,

            // Get API revision
            DdiOp::GetApiRev => 1002,

            // Get Device Info
            DdiOp::GetDeviceInfo => 1003,

            // Delete key
            DdiOp::DeleteKey => 1014,

            // Open key
            DdiOp::OpenKey => 1015,

            // Generate attestation report for key
            DdiOp::AttestKey => 1016,

            // RSA Modular Exponentiation
            DdiOp::RsaModExp => 1031,

            // RSA unwrap
            DdiOp::RsaUnwrap => 1035,

            // Get unwrapping RSA key
            DdiOp::GetUnwrappingKey => 1051,

            // ECC generate key pair
            DdiOp::EccGenerateKeyPair => 1061,

            // ECC sign
            DdiOp::EccSign => 1062,

            // AES Generate Key
            DdiOp::AesGenerateKey => 1071,

            // AES Encrypt/ Decrypt
            DdiOp::AesEncryptDecrypt => 1072,

            // ECDH key exchange
            DdiOp::EcdhKeyExchange => 1074,

            // HKDF Derive
            DdiOp::HkdfDerive => 1075,

            // KBKDF (SP800-108) Counter HMAC Derive
            DdiOp::KbkdfCounterHmacDerive => 1076,

            // HMAC with Sha
            DdiOp::Hmac => 1077,

            // Get establish cred encryption ECC key
            DdiOp::GetEstablishCredEncryptionKey => 1101,

            // Establish credential
            DdiOp::EstablishCredential => 1102,

            // Get session encryption ECC key
            DdiOp::GetSessionEncryptionKey => 1103,

            // Open Session
            DdiOp::OpenSession => 1104,

            // Close Session
            DdiOp::CloseSession => 1105,

            // Change PIN
            DdiOp::ChangePin => 1106,

            // Get Cert Chain Info
            DdiOp::GetCertChainInfo => 1108,

            // Get Certificate
            DdiOp::GetCertificate => 1109,

            // Test only opcodes below
            // Reset function to default state for testing
            DdiOp::ResetFunction => 2001,

            // Import key in DER format for testing including private keys
            DdiOp::DerKeyImport => 2002,

            // Get Perf Log Chunk
            DdiOp::GetPerfLogChunk => 2003,

            // Test action request
            DdiOp::TestAction => 2004,

            // Get Private Key request
            DdiOp::GetPrivKey => 2005,

            // SHA Digest
            DdiOp::ShaDigest => 2006,

            // Get Random Number using RNG
            DdiOp::GetRandomNumber => 2007,

            // Import private key in RAW format for testing
            DdiOp::RawKeyImport => 2008,
