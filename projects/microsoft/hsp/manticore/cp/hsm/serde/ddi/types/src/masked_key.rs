...
pub struct MaskedKeyAesLayout {
    ...
    /// Length of the post IV padding in bytes.
...
impl MaskedKeyLayout {
    ...
    /// Returns the length of the post metadata padding
    pub fn post_metadata_pad_len(&self) -> usize {
        self.post_metadata_pad_len
    }

    /// Returns the length of the encrypted key
    pub fn encrypted_key_len(&self) -> usize {
        self.encrypted_key_len
    }

    /// Returns the length of the post encrypted key padding
...
impl PreEncodeMaskedKeyType<'_> {
    ...
    /// Return an immutable slice of the iv data
    pub fn iv(&self) -> &[u8] {
        match self {
            PreEncodeMaskedKeyType::Aes(aes) => aes.iv(),
        }
    }

    /// Return a mutable slice of the iv data
    pub fn iv_mut(&mut self) -> &mut [u8] {
        match self {
            PreEncodeMaskedKeyType::Aes(aes) => aes.iv_mut(),
        }
    }
...
pub struct DdiMaskedKeyMetadata {
    ...
    /// BKS2 Key Number
    #[ddi(id = 4)]
    /// The BKS2 index indicating which BKS2 this key is.
