...
impl DdiRsaUnwrapReq {
    ...
    pub fn wrapped_blob_pre_encode(
        ...
        if len > input_array.data.len() || len > output_array.len() || len < rsa_size {
            return Err(MborEncodeError::InvalidLen);
        }

        // Change endianness for just the rsa size chunk
        reverse_copy(&mut output_array[..rsa_size], &input_array.data[..rsa_size]);

        // Copy rest of data
        output_array[rsa_size..input_array.len()]
            .copy_from_slice(&input_array.data[rsa_size..input_array.len()]);
