...
impl<T> VolatileCell<T> {
    ...
    /// Retrive the the mutable pointer
    #[inline(always)]
...
mod tests {
    ...
    #[test]
    fn test_volatilecell() {
        let val = VolatileCell::new(5);
        let five = val.get();
        assert_eq!(five, 5);

        val.set(432);
        assert_eq!(val.get(), 432);

        let vol_ptr = val.as_ptr();
        let value: &mut u32 = unsafe { &mut *vol_ptr };
