...
*/
  ...
  __STATIC_FORCEINLINE uint32_t __SXTB16_RORn(uint32_t op1, uint32_t rotate)
  {
      ...
      {
          uint32_t _ror = __ROR(op1, rotate);
          __ASM volatile("sxtb16 %0, %1" : "=r"(result) : "r"(_ror));
      }
#else
      else
      {
  ...
  __STATIC_FORCEINLINE uint32_t __SXTAB16_RORn(uint32_t op1, uint32_t op2, uint32_t rotate)
  {
      ...
      {
          uint32_t _ror = __ROR(op2, rotate);
          __ASM volatile("sxtab16 %0, %1, %2" : "=r"(result) : "r"(op1), "r"(_ror));
      }
#else
      else
      {
