...
#ifndef HW_ROT_FIRMWARE_KEY_MANIFEST_STATIC_H_
#define HW_ROT_FIRMWARE_KEY_MANIFEST_STATIC_H_

#include  "firmware/hw_rot_firmware_key_manifest.h"
...
/**
 * Constant initializer for the firmware key manifest RoT API.
 */
#define	HW_ROT_FIRMWARE_KEY_MANIFEST_API_INIT  { \
		.has_root_key = hw_rot_firmware_key_manifest_has_root_key, \
		.get_root_key_hash = hw_rot_firmware_key_manifest_get_root_key_hash, \
		.verify_root_key = hw_rot_firmware_key_manifest_verify_root_key, \
		.has_free_root_key_slots = hw_rot_firmware_key_manifest_has_free_root_key_slots, \
		.update_root_key = hw_rot_firmware_key_manifest_update_root_key, \
		.get_svn = hw_rot_firmware_key_manifest_get_svn, \
		.get_svn_bits = hw_rot_firmware_key_manifest_get_svn_bits, \
		.update_svn = hw_rot_firmware_key_manifest_update_svn, \
		.refresh_svn = hw_rot_firmware_key_manifest_refresh_svn, \
		.has_active_tenancy = hw_rot_firmware_key_manifest_has_active_tenancy, \
		.get_tenancy_counter = hw_rot_firmware_key_manifest_get_tenancy_counter, \
		.get_tenancy_grant_token = hw_rot_firmware_key_manifest_get_tenancy_grant_token, \
		.tenancy_transfer = hw_rot_firmware_key_manifest_tenancy_transfer \
	}


/**
 * Initialize a static instance of a HW RoT for the firmware key manifest.
 *
 ...
 * @param root_key_ptr The root key for the firmware key manifest.
 * @param secondary_root_key_ptr Optional secondary root key for the firmware key manifest.
 */
#define	hw_rot_firmware_key_manifest_static_init(fuses_ptr, fuse_reg_addr, ccs_ptr, root_key_ptr, \
	secondary_root_key_ptr)	{ \
		.base = HW_ROT_FIRMWARE_KEY_MANIFEST_API_INIT, \
		.fuses = fuses_ptr, \
		.fuse_regs = fuse_reg_addr, \
		.root_key = root_key_ptr, \
		.secondary_key = secondary_root_key_ptr \
	}


#endif	/* HW_ROT_FIRMWARE_KEY_MANIFEST_STATIC_H_ */
