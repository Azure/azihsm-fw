...
#ifndef AES_CBC_HSP_STATIC_H_
#define AES_CBC_HSP_STATIC_H_
 ...
 */
#define	AES_CBC_HSP_API_INIT	{ \
		.set_key = aes_cbc_hsp_set_key, \
		.clear_key = aes_cbc_hsp_clear_key, \
		.encrypt_data = aes_cbc_hsp_encrypt_data, \
		.decrypt_data = aes_cbc_hsp_decrypt_data, \
	}


/**
 * Initialize a static instance for running AES-CBC operations using the HSP AES hardware block.
 *
 ...
 */
#define	aes_cbc_hsp_static_init(aes_ptr, ccs_ptr, slot_id_arg)	{ \
		.base = AES_CBC_HSP_API_INIT, \
		.aes = aes_ptr, \
		.ccs = ccs_ptr, \
		.slot_id = slot_id_arg, \
	}
