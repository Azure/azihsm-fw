...
#ifndef RIOT_CORE_HSP_STATIC_H_
#define RIOT_CORE_HSP_STATIC_H_
...
#define	RIOT_CORE_HSP_API_INIT  { \
		...
		.generate_alias_key = riot_core_hsp_generate_alias_key, \
...
/**
 * Internal call to initialize a static instance for HSP DICE layer 0 handling.  This can be a
 * constant instance.
 *
 * There is no validation done on the arguments.
 *
 * @param api The API functions to use for for the instance.
 ...
 * and CSR.
 * @param device_id_pathlen The path length to use for the Device ID CA certificate.  Use
 * X509_CERT_MAX_PATHLEN to indicate the certificate should be generated without any path length
 * constraint.
 ...
 * path length constraint.  Use -1 to indicate the Alias will not be a CA certificate.
 */
#define	riot_core_hsp_static_init_with_api(api, state_ptr, ccs_ptr, base64_ptr, x509_ptr, \
	cdi_slot, device_id_slot, alias_slot, device_id_ext_ptr, device_id_ext_cnt, device_id_pathlen, \
	alias_ext_ptr, alias_ext_cnt, alias_pathlen_arg)	{ \
		.base = api, \
		.state = state_ptr, \
		.ccs = ccs_ptr, \
		.base64 = base64_ptr, \
		...
		.dev_id_pathlen = device_id_pathlen, \
		.alias_ext = alias_ext_ptr, \
		.alias_ext_count = alias_ext_cnt, \
		.alias_pathlen = alias_pathlen_arg, \
		.cdi = cdi_slot, \
		.device_id_key = device_id_slot, \
		.alias_key = alias_slot \
	}


/**
 * Initialize a static instance for HSP DICE layer 0 handling.  This implementation assumes the
 * Device ID key has already been generated, presumably by ROM, and is present in the KSU along with
 * the CDI.  This can be a constant instance.
 *
 * There is no validation done on the arguments.
 *
 * @param state_ptr Variable context for DICE handling.
 ...
 * @param alias_ext_cnt The number of custom extensions to add to the Alias certificate.
 */
#define	riot_core_hsp_static_init(state_ptr, ccs_ptr, base64_ptr, x509_ptr, cdi_slot, \
	device_id_slot, alias_slot, device_id_ext_ptr, device_id_ext_cnt, device_id_pathlen, \
	alias_ext_ptr, alias_ext_cnt) \
		riot_core_hsp_static_init_with_api (RIOT_CORE_HSP_API_INIT, state_ptr, ccs_ptr, \
			base64_ptr, x509_ptr, cdi_slot, device_id_slot, alias_slot, device_id_ext_ptr, \
			device_id_ext_cnt, device_id_pathlen, alias_ext_ptr, alias_ext_cnt, -1)
 ...
 * path length constraint.
 */
#define	riot_core_hsp_static_init_alias_ca(state_ptr, ccs_ptr, base64_ptr, x509_ptr, cdi_slot, \
	device_id_slot, alias_slot, device_id_ext_ptr, device_id_ext_cnt, device_id_pathlen, \
	alias_ext_ptr, alias_ext_cnt, alias_pathlen_arg) \
		riot_core_hsp_static_init_with_api (RIOT_CORE_HSP_API_INIT, state_ptr, ccs_ptr, \
			base64_ptr, x509_ptr, cdi_slot, device_id_slot, alias_slot, device_id_ext_ptr, \
			device_id_ext_cnt, device_id_pathlen, alias_ext_ptr, alias_ext_cnt, alias_pathlen_arg)


#endif	/* RIOT_CORE_HSP_STATIC_H_ */
