...
#include "boot_measurements_dual_root.h"


/**
 * Generate the measurement log for firmware images loaded with dual root manifests.
 *
	...
	const struct key_manifest_hsp_rom_dual_root *keys, const struct hsp_fw_1sp *fw)
{
	...
	memset (log, 0, sizeof (struct boot_measurements_dual_root_log));
	...
	key = keys->get_root_authority_key (keys);
	status = boot_measurements_generate_log_public_key (&log->authority_public_key, hash, key,
		BOOT_MEASUREMENTS_EVENT_SECONDARY_OWNER_PUBLIC_KEY);
	if (status != 0) {
		return status;
	}

	status = boot_measurements_generate_log_manifest_svn (&log->key_manifest_svn, hash, &keys->base,
		hw_key);
	if (status != 0) {
		return status;
	}
	...
	status = boot_measurements_generate_log_primary_firmware_key (&log->fw_public_key, hash,
		&keys->base);
	if (status != 0) {
		return status;
	}
	...
	status = boot_measurements_generate_log_tenancy_grant_key (&log->grant_public_key, hash,
		&keys->base, keys->base.is_tenancy_grant (&keys->base));
	if (status != 0) {
		return status;
	}
...
/**
 * Extend a PCR using digests from a measurement log for dual root manifests.
 *
	...
	const struct ccs_ksu_interface *ccs, uint8_t pcr)
{
	...
	status = ccs->extend_pcr (ccs, pcr, &log->authority_public_key.digest);
	if (status != 0) {
		return status;
	}
	...
	status = ccs->extend_pcr (ccs, pcr, &log->grant_public_key.digest);
	if (status != 0) {
		return status;
	}
