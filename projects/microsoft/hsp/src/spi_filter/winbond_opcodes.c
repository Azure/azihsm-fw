...
/**
 * SPI filter opcode configuration for Winbond W25Q devices with a capacity of no more than 128Mbit.
 * These devices only support 3-byte address mode.
 */
const union spi_filter_hsp_opcode winbond_opcodes_w25q128[] = {
	{
...
/**
 * The number of filtered opcodes for Winbond W25Q devices with a capacity of no more than 128Mbit.
 */
const size_t winbond_opcodes_w25q128_count = ARRAY_SIZE (winbond_opcodes_w25q128);

/**
 * SPI filter opcode configuration for Winbond W25Q devices with a capacity of at least 256Mbit.
 * These devices support 3 and 4-byte address mode.
 *
 * There are slightly different opcodes supported between the FV and JV versions of this device, but
 * it is not possible to differentiate between them based on device ID.  The opcode table is
 * generated based on command support in the newer, JV version.
 */
const union spi_filter_hsp_opcode winbond_opcodes_w25q256[] = {
	{
		.opcode = 0x02,	// 1-1-1 Page Program
		.action = SPI_FILTER_HSP_FILTER_STATE_WRADDR_1X1X1 | SPI_FILTER_HSP_FILTER_BIT_8
	},
	...
	{
		.opcode = 0x34,	// 1-1-4 4B Page Program
		.action = SPI_FILTER_HSP_FILTER_STATE_WRADDR4B_1X1X4 | SPI_FILTER_HSP_FILTER_BIT_7
	},
	{
		.opcode = 0x35,	// Read Status Register 2
		.action = SPI_FILTER_HSP_FILTER_STATE_INACTIVE | SPI_FILTER_HSP_FILTER_BIT_7
...
/**
 * The number of filtered opcodes for Winbond W25Q devices with a capacity of at least 256Mbit.
 */
const size_t winbond_opcodes_w25q256_count = ARRAY_SIZE (winbond_opcodes_w25q256);
